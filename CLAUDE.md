...
- **`animals_game`** — Bevy 2D visualizer. In `--ai` mode it picks a free ephemeral TCP port, spawns `learner.play` as a child process (killed via `Drop` on `AiServerProcess`), and exchanges raw little-endian bytes per tick: `num_snakes * SNAKE_OBS_SIZE + total_preys * PREY_OBS_SIZE` f32 observations out, `num_snakes + total_preys` i32 actions back. AI-driven ticks call `engine.step()` then `engine.respawn_dead()` then `engine.respawn_dead_preys()` (matching the training simulation's per-tick respawn, so `game_tick`'s "all snakes dead -> game_over" check effectively never fires in AI mode — it's an aquarium that keeps running). Manual (non-AI) ticks skip `respawn_dead()`: the engine no longer sets `game_over` itself, so `game_tick` sets it when it sees a dead snake, preserving the "freeze, press Space to restart" UX for keyboard play.
...
1. `animals_engine/src/lib.rs` — `SNAKE_OBS_SIZE`, `PREY_OBS_SIZE`, `HUNGER_LIMIT`, `SMELL_RANGE`, `VISIT_HORIZON` constants (used by `game.rs`'s two observation functions, `update_targets`, and the visitation-tracking pass in `step`)
