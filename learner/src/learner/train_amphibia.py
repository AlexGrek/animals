...
def main():
    ...
    parser.add_argument("--amphibias-per-game", type=int, default=16, help="Number of amphibias per game instance.")
    parser.add_argument("--max-amphibias", type=int, default=100, help="Max amphibias per game instance.")
    ...
    try:
        ...
        logger.info(f"Creating Amphibia Vector Env with {args.num_games} games across {args.num_procs} process(es) ({threads} torch threads/process)...")

        def make_env_fn(proc_idx):
            def _init():
                return RustAmphibiaVecEnv(
        ...
        if args.resume and os.path.exists(args.model_path):
            logger.info(f"Resuming Amphibia training from {args.model_path}...")
