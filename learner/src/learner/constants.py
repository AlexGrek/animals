...
Keep these in sync with `SNAKE_OBS_SIZE`, `PREY_OBS_SIZE`, and `HUNGER_LIMIT`
there — changing the observation size invalidates saved checkpoints in
`learner/models/` (SB3 load fails on shape mismatch).
"""

# 8x8 grid (64) + prey unit-direction (2) + normalized distance (1)
